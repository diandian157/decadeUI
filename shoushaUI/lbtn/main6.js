...
        jiaojia.innerHTML = "<span style='letter-spacing:-1px;font-weight:400;'>城主出兵</span>" + "<br>" + (num = ['100', '200', '300', '600', '900']).randomGet(1);
        jiaojia.style.cssText = "display: line;position: absolute;top: 55px;color: #523a29;left: 150px;font-size:15px;font-family:yuanli; z-index:1; text-shadow: #000 0 0 0.3px,#000 0 0 0.3px,#000 0 0 0.3px;-webkit-text-stroke:none;font-weight:400;"
        document.body.appendChild(jiaojia);
*/
				var head = ui.create.node("img");
				head.src = lib.assetURL + "extension/十周年UI/shoushaUI/lbtn/images/uibutton/出兵/" + (num = [1, 2, 3]).randomGet(1) + ".png";
				head.style.cssText = "display: block;height:25px;width:100px;position: absolute;top: 64px;left:134px;background-size:cover;background-color: transparent;z-index:-1";
				document.body.appendChild(head);
				var head = ui.create.node("img");
				head.src = lib.assetURL + "extension/十周年UI/shoushaUI/lbtn/images/uibutton/shoupai.png";
				head.style.cssText = "display: block; --w: 60px;--h: 50px;width: var(--w);height: var(--h);position: absolute;top: 5px;left:136px;background-color: transparent;z-index:-1";
				document.body.appendChild(head);
			}
		}
		...
		if (lib.config["extension_十周年UI_rightLayout"] == "on") {
			head.style.cssText =
				"display: block;--w: 88px;--h: calc(var(--w) * 81/247);width: var(--w);height: var(--h);position: absolute;top: calc(100% - 35px);left: calc(100% - 340px);background-color: transparent;z-index:2";
		} else {
			head.style.cssText =
		...
		create: {
			...
			handcardNumber() {
				...
				node.updateCardnumber = function() {
					...
					var cardNumber2 = game.me.countCards("h") || 0; //获取当前角色手牌数，未定义是0
					var cardNumber = game.me.getHandcardLimit() || 0; //获取当前角色手牌上限，未定义是0
					var numbercolor = "white";
					if (cardNumber2 > cardNumber) numbercolor = "red";
					if (cardNumber == Infinity) cardNumber = "∞";
					this.node.cardNumber.innerHTML = "<span>" + "<font color:#FFF>" + cardNumber2 + "</font>" + '<sp style="font-size:20px; font-family:yuanli; color:#FFF;">' + " / " + "</sp>" + "<font color=#FF0000>" + cardNumber + "</font>" + "</span>"; /*手牌数参数*/ /**/ //      this.setNumberAnimation(cardNumber);
					this.show();
