...
try {
	const plugin = createSkillPlugin(window.lib, window.game, window.ui, window.get, window.ai, window._status, window.app);
	if (plugin) {
		...
		console.log("[十周年UI] Skill模块加载成功");
	}
} catch (e) {
	console.error("[十周年UI] Skill模块加载失败:", e);
}
